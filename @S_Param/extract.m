...
% S_Param extract will either extract as an S_Param object if possible
    ...
    if obj.is_mixed
        ...
        if ~isempty(m)
            ...
            if isempty(ij)
                ...
                P1 = EMC.S_Param(Smm.Freq,Smm.Data(i,j,:),Smm.Impedance,Smm.UnitF,Smm.Unit);
            end
        else
        ...
        if ~isempty(m)
            ij = m{1}{1};
            i = str2double(ij(1)); j = str2double(ij(2));
            P1 = EMC.S_Param(obj.Freq,obj.Data(i,j,:),obj.Impedance,obj.UnitF,obj.Unit);
